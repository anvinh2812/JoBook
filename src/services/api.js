...
export const cvsAPI = {
  getCVs: () => api.get('/cvs'),
  uploadCV: (formData, config = {}) => api.post('/cvs/upload', formData, config),
  toggleCV: (id) => api.patch(`/cvs/${id}/toggle`),
  deleteCV: (id) => api.delete(`/cvs/${id}`),
  getCVFile: (id) => api.get(`/cvs/${id}/file`, { responseType: 'blob' }),

  // ✅ Sửa lại để log đúng & tránh crash
  getCVText: async (id) => {
    try {
      const { data } = await api.get(`/cvs/${id}/text`);
      return data.text || null;
    } catch (err) {
      if (err.response?.status === 404) {
        console.warn(`⚠️ Không tìm thấy nội dung CV (id=${id})`);
        return null;
      }
      console.error("❌ Lỗi khi lấy CV text:", err?.response?.data || err.message);
      return null;
    }
  },
